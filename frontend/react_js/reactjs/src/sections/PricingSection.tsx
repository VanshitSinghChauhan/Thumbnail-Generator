        ...
        <div id="pricing" className="px-4 md:px-16 lg:px-24 xl:px-32">
            ...
            <div className="flex flex-wrap items-center justify-center gap-8 mt-20">
                {pricingData.map((plan: IPricing, index: number) => (
                    <motion.div key={index} className={`w-72 text-center border border-orange-950 p-6 pb-16 rounded-xl ${plan.mostPopular ? 'bg-orange-950 relative' : 'bg-orange-950/30'}`}
                    ...
                    >
                        {plan.mostPopular && (
                            <p className="absolute px-3 text-sm -top-3.5 left-3.5 py-1 bg-orange-400 rounded-full">Most Popular</p>
                        )}
                        ...
                        <button type="button" className={`w-full py-2.5 rounded-md font-medium mt-7 transition-all ${plan.mostPopular ? 'bg-white text-orange-600 hover:bg-slate-200' : 'bg-orange-500 hover:bg-orange-600'}`}>
                            Get Started
                        </button>
                    </motion.div>
                ))}
